...
`index_series()` rescales a series to a common base, either using its earliest
observation or the mean over a selected year or date range. It also supports
grouped data and multiple value columns.
...
The package includes daily Arabica and Robusta coffee price indicators from
the Centro de Estudos Avançados em Economia Aplicada (CEPEA), Escola Superior
de Agricultura Luiz de Queiroz (ESALQ), Universidade de São Paulo (USP). See
the [Arabica](https://www.cepea.org.br/br/indicador/series/cafe.aspx?id=23) and
[Robusta](https://www.cepea.org.br/br/indicador/series/cafe.aspx?id=24) source
series, and the [Arabica methodology](https://www.cepea.org.br/br/metodologia/metodologia-cafe-arabica-cepea-esalq.aspx)
and [Robusta methodology](https://www.cepea.org.br/br/metodologia/metodologia-cafe-robusta-cepea-esalq.aspx).
...
That license applies to the CEPEA-derived data; the package code is licensed
under MIT. The bundled data are an adapted version: `usd_2022` is calculated
from the source dollar price using U.S. inflation data, and `trend_ma` is a
22-observation moving-average column. The first observations are missing while
the moving-average window fills.
