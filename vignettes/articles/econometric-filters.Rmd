...
adjustment programs X-11, X-13ARIMA-SEATS (US Census Bureau), and Statistics
Canada's X-12-ARIMA. Robert Henderson (1916) proposed it to smooth actuarial
mortality tables, and it became standard for national accounts trend
extraction. Two properties explain why.
...
the series. For monthly indicators, 13-term is a reasonable starting point.
...
just like the Henderson filter. The endpoints use linear extrapolation, so the
Spencer filter returns a trend for every observation.
...
On the same chart the two filters look very similar. Henderson's practical
advantage is that its length can be chosen to match the irregularity of the
series. The `augment_trends` function accepts multiple methods and returns a
column for each.
...
frequency domain. The key parameter is `band = c(pl, pu)`, the lower and upper
period bounds (in quarters by default). The default `c(6, 32)` targets cycles
of 1.5 to 8 years, the standard macroeconomic business cycle definition.
...
The two filters track each other closely in the interior of the sample. CF also
estimates the endpoints, which makes it the safer default when the sample is
short or when recent values matter.
...
Hamilton (2018) recommends two years ahead and one year of lags: $h = 24$,
$p = 12$ for monthly data and $h = 8$, $p = 4$ for quarterly data. For
quarterly data the regression written out in full is
...
> concern (e.g., near real-time analysis), or when you want a trend that is not
> exposed to the critiques in Hamilton (2018). It is also easy to interpret —
> the trend is a projection of the future level onto past values.
>
...
`augment_trends()` applies several filters in a single call. The comparison
below uses the IBC-Br after 2010, where every filter has enough data.
