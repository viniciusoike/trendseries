...
This article catalogues the trend-extraction methods in `trendseries`: what each
one does, when to reach for it, and which parameters it accepts.
For worked examples of specific filters, see the companion
[Moving Averages](moving-averages.html) and
[Econometric Filters](econometric-filters.html) articles. To split a series
...
Two functions share the same engine and the same parameters, and either one
...
they correspond to. Each one has a frequency-aware default, so you can leave
them unset.
...
quarterly series, `lambda = 1600` and a window of 4.
...
Moving-average methods replace each point with a (possibly weighted) average of its neighbours, which makes them a good starting point for exploratory work. Control the smoothing through `window` (and `align` for 'left', 'center', or 'right'). `ma`, `median`, and `henderson` also accept a *vector* of windows, returning one trend per window.
...
Smoothing methods fit a flexible curve to the data. Methods like `stl` and `loess` are locally adaptive; `spline` and `kernel` trade off fit against smoothness through a penalty/bandwidth; `poly` imposes a single global shape. The `smoothing` parameter tunes how aggressively they smooth.
...
These are the typical filters of applied macroeconomics. The Hodrick-Prescott filter (`hp`) is perhaps the most widely used; `hamilton` is a regression-based alternative that attempts to avoid HP's well-known spurious-cycle artefacts; `bn` and `ucm` are model-based decompositions into permanent and transitory parts.
...
The dedicated [Decomposing Series](decompose-series.html) article covers these
in depth.
