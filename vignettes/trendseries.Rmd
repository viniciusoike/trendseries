...
`stats::stl`). We also need to inform the names of the date column
("date" as default) and the value column ("value) as default.
...
```{r}
ggplot(retail_autofuel, aes(date, automotive_fuel)) +
...
comes at a cost of lost of precision. For instance, the `stats::stl` function has both a `t.window` and `s.window` arguments. The `window` argument assumes the user wants to control the `s.window` argument. This case illustrates how trendseries tries to simplify the workflow by being opinionated about default user choices.
...
For instance, building an HP filter estimate of `gdp_construction`, without
`trendseries` requires one to first convert the data frame to a `ts`
...
And finally, converting it back to a `data.frame` and merging it with the
original data.
