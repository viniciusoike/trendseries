...
electric_decomp <- electric_stl |>
  mutate(
    seasonal = consumption - trend_stl,  # Approximate seasonal (for visualization)
    trend = trend_stl
  )

# Plot original and trend
p1 <- electric_decomp |>
  select(date, consumption, trend) |>
  pivot_longer(cols = c(consumption, trend), names_to = "series") |>
  mutate(series = ifelse(series == "consumption", "Original", "Trend")) |>
...
# Plot comparison
electric_comparison |>
  select(date, consumption, trend_stl, trend_hp, trend_ma) |>
  pivot_longer(
    cols = c(consumption, starts_with("trend_")),
    names_to = "method",
    values_to = "value"
  ) |>
  mutate(
    method = case_when(
      method == "consumption" ~ "Original",
...
for (s in smoothing_levels) {
  temp <- vehicles_recent |>
    augment_trends(value_col = "production", methods = "kalman", smoothing = s) |>
    select(trend_kalman)
...
# Plot
vehicles_kalman |>
  select(date, production, starts_with("kalman_")) |>
  pivot_longer(
    cols = c(production, starts_with("kalman_")),
    names_to = "method",
    values_to = "value"
  ) |>
  mutate(
...
# Plot
vehicles_sg_poly |>
  select(date, production, sg_order2, sg_order3, trend_sg) |>
  pivot_longer(
    cols = c(production, sg_order2, sg_order3, trend_sg),
    names_to = "method",
    values_to = "value"
  ) |>
  mutate(
...
# Plot
ibcbr_loess |>
  select(date, index, trend_loess) |>
  pivot_longer(cols = c(index, trend_loess), names_to = "series") |>
  mutate(series = ifelse(series == "index", "Original", "LOESS Trend")) |>
  ggplot(aes(x = date, y = value, color = series)) +
  geom_line(linewidth = 0.8) +
  labs(
...
for (span in spans) {
  temp <- ibcbr_recent |>
    augment_trends(value_col = "index", methods = "loess", smoothing = span) |>
    select(trend_loess)
...
# Plot
ibcbr_spans |>
  select(date, index, starts_with("loess_")) |>
  pivot_longer(
    cols = c(index, starts_with("loess_")),
    names_to = "method",
    values_to = "value"
  ) |>
  mutate(
...
end_sample |>
  select(date, index, starts_with("trend_"), recent) |>
  pivot_longer(
    cols = c(index, starts_with("trend_")),
    names_to = "method",
    values_to = "value"
  ...
  annotate("text", x = end_sample$date[19], y = max(end_sample$index, na.rm = TRUE),
