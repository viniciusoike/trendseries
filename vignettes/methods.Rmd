...
This vignette catalogues the trend-extraction methods in `trendseries`: which
family each belongs to, when to reach for it, and which parameters it accepts.
For worked examples of specific families, see the companion
...
`stl` and `loess` are locally adaptive; `spline` and `kernel` trade off fit against smoothness through
...
These are the typical filters of applied macroeconomics. The Hodrick-Prescott filter
