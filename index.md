...
All of these issues take away time from the typical workflow of data analysis. `trendseries` aims to make this all simpler by providing a single function `augment_trends` that adds estimated trends as new a column to a dataset. When possible, it also tries to offer standard parameter names.

`trendseries` is designed for **exploratory time series analysis**, with a primary focus on monthly and quarterly economic data. It prioritizes simplicity and consistency. It should be mentioned that certain methods (STL, moving averages, LOESS, etc.) can handle daily and complex frequencies.
...
The package provides two main functions: `augment_trends()` for data.frame objects and `extract_trends()` for time series objects.
...
Note that `stl` has both a `t.window` and `s.window` parameters, but `trendseries` assumes the users wishes to choose the latter and not the former. This case illustrates how `trendseries` tries to simplify the workflow by being opinionated about default user choices.
...
`augment_trends()` can also work with multiple value columns.
...
With `trendseries`, as seen above, this can be accomplished in a single step.
...
Currently, a downside of `trendseries` is that it returns only the "trend" estimate, ignoring the "remainder" and "seasonal" components.
...
The goal of `trendseries` is to facilitate exploratory data analysis of time
series data in a conventional `data.frame` format. The package offers popular
econometric and statistical method for trend decomposition and tries to unify
parameter names when possible.
